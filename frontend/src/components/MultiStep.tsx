    ...
    return  <>
                <NavBar variant="primary">
                    {clickableStepsKeys.map(title => {
                        return <Button onClick={()=>{setStep(clickableStepsKeys.indexOf(title)+1)}} disabled={!clickableSteps.get(title)}>{title}</Button>
                    })}
                </NavBar>
                ...
                <div>
                    ...
                    <Button onClick={()=>setStep(Math.min(clickableStepsKeys.length, step + 1))} disabled={!clickableSteps.get(clickableStepsKeys[step])}>Suivant</Button>
                </div>
            </>
}